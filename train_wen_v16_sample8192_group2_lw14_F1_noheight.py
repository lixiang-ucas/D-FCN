...
# print(reader.get_tensor('layer0_c0/c0_conv1/weights').shape)
...
def eval_one_epoch_whole_scene2(sess, ops):
    """ ops: dict mapping from string to tf ops """
    is_training = False
    total_correct = 0
    total_seen = 0
...
#         aug_data = provider.rotate_point_cloud_z(batch_data)
        ...
        NUM_POINT_ori = batch_data.shape[1]
        for i in range(BATCH_SIZE):
            for j in range(NUM_POINT_ori):
                l = batch_label[i, j]
                total_seen_class[l] += 1
                total_correct_class[l] += (pred_val[i, j] == l)
        ...
        pre_list.append(pred_val)
        label_list.append(batch_label)
        
    C = np.sum(np.stack(Confs), axis=0).astype(np.float32)
    Acc_from_confusions(C)
...
#     log_string('learn rate: %f' % (lr_val))
    ...
    return mloss, macc, C, pre_list, label_list
