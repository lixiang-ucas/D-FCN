...
__global__ void gatherpoint2Kernel(int b,int n,int m,const float * __restrict__ inp,const int * __restrict__ idx,float * __restrict__ out){
  for (int i=blockIdx.x;i<b;i+=gridDim.x){
    for (int j=blockIdx.y*blockDim.x+threadIdx.x;j<m;j+=blockDim.x*gridDim.y){
      int a=idx[i*m+j];
      out[(i*m+j)*6+0]=inp[(i*n+a)*6+0];
      out[(i*m+j)*6+1]=inp[(i*n+a)*6+1];
      out[(i*m+j)*6+2]=inp[(i*n+a)*6+2];
      out[(i*m+j)*6+3]=inp[(i*n+a)*6+3];
      out[(i*m+j)*6+4]=inp[(i*n+a)*6+4];
      out[(i*m+j)*6+5]=inp[(i*n+a)*6+5];
    }
  }
}
...
void gatherpoint2Launcher(int b,int n,int m,const float * inp,const int * idx,float * out){
  gatherpoint2Kernel<<<dim3(2,8,1),512>>>(b,n,m,inp,idx,out);
}
