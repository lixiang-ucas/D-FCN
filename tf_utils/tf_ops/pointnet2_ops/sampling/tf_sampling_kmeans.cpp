...
// void kmeanstpointsamplingLauncher(int b,int n,int m,const float * inp,float * temp,int * out);
// class KmeanstPointSampleGpuOp: public OpKernel{
//   public:
//     explicit KmeanstPointSampleGpuOp(OpKernelConstruction* context):OpKernel(context) {
//                     OP_REQUIRES_OK(context, context->GetAttr("npoint", &npoint_));
//                     OP_REQUIRES(context, npoint_ > 0, errors::InvalidArgument("KmeansPointSample expects positive npoint"));
//                 }
//     void Compute(OpKernelContext * context)override{
//       int m = npoint_;

//       const Tensor& inp_tensor=context->input(0);
//       OP_REQUIRES(context,inp_tensor.dims()==3 && inp_tensor.shape().dim_size(2)==3,errors::InvalidArgument("KmeansPointSample expects (batch_size,num_points,3) inp shape"));
//       int b=inp_tensor.shape().dim_size(0);
//       int n=inp_tensor.shape().dim_size(1);
//       auto inp_flat=inp_tensor.flat<float>();
//       const float * inp=&(inp_flat(0));
...
//       auto temp_flat=temp_tensor.flat<float>();
//       float * temp=&(temp_flat(0));
//       meanspointsamplingLauncher(b,n,m,inp,temp,out);
//     }
//     private:
//         int npoint_;
// };
// REGISTER_KERNEL_BUILDER(Name("KmeansPointSample").Device(DEVICE_GPU),KmeanstPointSampleGpuOp);
