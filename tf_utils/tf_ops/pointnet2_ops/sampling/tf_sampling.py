...
returns:
    batch_size * npoints * 3    float32
    '''
    return sampling_module.gather_point2(inp,idx)
#@tf.RegisterShape('GatherPoint')
#def _gather_point_shape(op):
#    shape1=op.inputs[0].get_shape().with_rank(3)
#    shape2=op.inputs[1].get_shape().with_rank(2)
...
# def kmeans_point_sample(npoint,inp):
#     '''
# input:
#     int32
#     batch_size * ndataset * 3   float32
...
#     batch_size * npoint         int32
#     '''
#     return sampling_module.kmeans_point_sample(inp, npoint)
# ops.NoGradient('KmeanstPointSample')
...
if __name__=='__main__':
    ...
    with tf.device('/gpu:1'):
        ...
        idx = farthest_point_sample(1024,pt_sample)
        print 'pt_sample: ', pt_sample
        pt_sample = tf.concat([pt_sample,pt_sample],-1)
        print 'pt_sample: ', pt_sample
        reduced_sample=gather_point2(pt_sample,idx)
