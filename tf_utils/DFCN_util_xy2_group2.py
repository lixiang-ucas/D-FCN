...
sys.path.append(os.path.join(BASE_DIR, 'tf_ops/DFCN_op'))

from DFCN_op import DFCN_select, DFCN_select_two, DFCN_select_four
...
def DFCN_group(radius, xyz, points, use_xyz=True):
    idx = DFCN_select(xyz, radius)
...
def DFCN_group_with_idx(xyz, idx, points, use_xyz=True):
    grouped_xyz = group_point(xyz, idx)  # (batch_size, npoint, 8, 3)
    grouped_xyz -= tf.tile(tf.expand_dims(xyz, 2), [1, 1, 8, 1])  # translation normalization
    if points is not None:
...
def DFCN_group_two(radius, xyz, points, use_xyz=True):
    idx = DFCN_select_two(xyz, radius)
...
def DFCN_group_two_with_idx(xyz, idx, points, use_xyz=True):
...
def DFCN_group_four(radius, xyz, points, use_xyz=True):
    idx = DFCN_select_four(xyz, radius)
    grouped_xyz = group_point(xyz, idx)  # (batch_size, npoint, 32, 3)
...
def DFCN_group_four_with_idx(xyz, idx, points, use_xyz=True):
    grouped_xyz = group_point(xyz, idx)  # (batch_size, npoint, 8/32, 3)
    grouped_xyz -= tf.tile(tf.expand_dims(xyz, 2), [1, 1, 32, 1])  # translation normalization
    if points is not None:
        grouped_points = group_point(points, idx)  # (batch_size, npoint, 8/32, channel)
...
def DFCN_module(xyz, points, radius, out_channel, is_training, bn_decay, scope='point_DFCN', bn=True, use_xyz=True, use_nchw=False):
    data_format = 'NCHW' if use_nchw else 'NHWC'
    with tf.variable_scope(scope) as sc:
        # Grouping
        new_xyz, new_points, idx, grouped_xyz = DFCN_group_two(radius, xyz, points, use_xyz)
        ...
        for i in range(2):
            if i==0:
                ...
                new_points = tf_util.conv2d(new_points, out_channel, [1, 8],
                                            padding='VALID', stride=[1, 8],
                                            bn=bn, is_training=is_training,
                                            scope='conv%d' % (i), bn_decay=bn_decay,
                                            data_format=data_format)
        # add fc
...
def DFCN_res_module(xyz, points, radius, out_channel, is_training, bn_decay, scope='point_DFCN', bn=True, use_xyz=True, same_dim=False, merge='add'):
    data_format = 'NHWC'
    with tf.variable_scope(scope) as sc:
        # conv1
        _, new_points, idx, _ = DFCN_group_two(radius, xyz, points, use_xyz=use_xyz) #DFCN_group
        ...
        # conv2
        _, new_points, idx, _ = DFCN_group_two_with_idx(xyz, idx=idx, points=new_points, use_xyz=use_xyz)
