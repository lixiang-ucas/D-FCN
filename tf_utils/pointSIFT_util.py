...
from DFCN_op import DFCN_select, DFCN_select_four
...
def DFCN_module(xyz, points, radius, out_channel, is_training, bn_decay, scope='point_DFCN', bn=True, use_xyz=True, use_nchw=False):
    ...
    with tf.variable_scope(scope) as sc:
        # Grouping
        new_xyz, new_points, idx, grouped_xyz = DFCN_group(radius, xyz, points, use_xyz)
...
def DFCN_res_module(xyz, points, radius, out_channel, is_training, bn_decay, scope='point_DFCN', bn=True, use_xyz=True, same_dim=False, merge='add'):
    ...
    with tf.variable_scope(scope) as sc:
        # conv1
        _, new_points, idx, _ = DFCN_group(radius, xyz, points, use_xyz=use_xyz) #DFCN_group
        ...
        # conv2
        _, new_points, idx, _ = DFCN_group_with_idx(xyz, idx=idx, points=new_points, use_xyz=use_xyz)
